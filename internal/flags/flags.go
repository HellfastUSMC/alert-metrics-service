...
func ParseServerAddr() {
	flag.StringVar(&ServerAddr, "a", "http://localhost:8080", "Address and port of server")
	flag.Parse()
}

func ParseAgentFlags() {
	agentFlags := flag.NewFlagSet("agent flags", flag.ExitOnError)
	agentFlags.StringVar(&AgentServerAddr, "a", "http://localhost:8080", "Address and port of server")
